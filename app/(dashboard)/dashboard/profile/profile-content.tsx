...
export function ProfileContent({ user }: ProfileContentProps) {
  ...
  const handleCropComplete = useCallback(async (blob: Blob) => {
    ...
    startAvatarTransition(async () => {
      await uploadAvatarAction(null, formData);
    });
  }, []);
      ...
      <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
        ...
        <Card className="border border-gray-200/80 dark:border-gray-800/80 rounded-[24px] bg-white dark:bg-[#16161D]">
          <div className="p-6">
            ...
            {saveError && (
              <div className="mb-4 rounded-xl bg-red-50 dark:bg-red-950/30 p-3 text-sm text-red-600">
                {saveError}
              </div>
            )}
            {saveSuccess && (
              <div className="mb-4 rounded-xl bg-green-50 dark:bg-green-950/30 p-3 text-sm text-green-600">
            ...
            <form ref={formRef} onSubmit={handleSave} className="space-y-5">
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    disabled
                    className="rounded-xl h-11 bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 opacity-60"
                  />
                </div>
                <div className="space-y-2">
                  <Label className="flex items-center gap-2 text-[13px] font-medium text-gray-600 dark:text-gray-400">
              ...
              {/* Location — replaces old City/Area/Country plain inputs */}
              <div>
                <button
                  type="button"
                  onClick={() => setShowLocation(!showLocation)}
                  className="flex items-center gap-2 text-sm font-semibold text-[#0066FF] hover:text-[#0052CC] transition-colors mb-3"
                >
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <Button
                  type="submit"
                  className="rounded-xl bg-[#0066FF] hover:bg-[#0052CC] text-white px-6 h-10 shadow-lg shadow-blue-500/20 disabled:opacity-50"
                  disabled={isSaving}
                >
                  {isSaving ? (

...
export function ProfileForm({ user }: ProfileFormProps) {
  ...
  const [avatarState, avatarFormAction, isUploading] = useActionState(uploadAvatarAction, {});
  ...
  // When crop is done, auto-upload via server action
  const handleCropComplete = useCallback(async (blob: Blob) => {
    const url = URL.createObjectURL(blob);
    setAvatarPreview(url);
    setShowCrop(false);

    // Auto-upload via server action
          ...
          <CardContent className="p-8 text-center">
            <div className="relative inline-block group cursor-pointer" onClick={handleAvatarClick}>
              ...
              <div className="absolute inset-0 rounded-full bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                {isUploading ? <Loader2 className="size-6 text-white animate-spin" /> : <Camera className="size-6 text-white" />}
              </div>
            </div>
            ...
            {isUploading && <p className="mt-3 text-xs text-gray-400 flex items-center justify-center gap-1"><Loader2 className="size-3 animate-spin" /> Uploading...</p>}
        ...
        <Card className="lg:col-span-2 border border-gray-100 dark:border-gray-800 rounded-[24px] bg-white dark:bg-gray-900">
          ...
          <CardContent className="space-y-5">
            <div className="grid gap-5 sm:grid-cols-2">
              <div className="space-y-2"><Label className="flex items-center gap-2 text-[13px] font-medium text-gray-600 dark:text-gray-400"><User className="size-3.5" /> Full Name</Label><Input defaultValue={user.full_name} className="rounded-xl h-11 bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 focus:border-[#0066FF] focus:ring-[#0066FF]/20" /></div>
            ...
            <div className="flex justify-end pt-2"><Button className="rounded-full bg-[#0066FF] hover:bg-[#0052CC] text-white px-8 h-11 shadow-lg shadow-blue-500/20" disabled={saving}>{saving ? <Loader2 className="mr-2 size-4 animate-spin" /> : null} Save Changes</Button></div>
          </CardContent>
        </Card>
      </div>
